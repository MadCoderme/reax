...
app.get('/readFile', cors(corsOptions), (req, res) => {
  const content = fs.readFileSync('./build/src/apps/' + req.query.path, 'utf8')
  res.send(content)
})
...
app.get('/writeFile', cors(corsOptions), (req, res) => {
  ...
  if(req?.query?.type === 'append') {
    fs.appendFileSync('./build/src/apps/' + req.query.path, req.query.content, { encoding })
  } else {
    fs.writeFileSync('./build/src/apps/' + req.query.path, req.query.content, { encoding })
  }
...
app.post('/writeFile', cors(corsOptions), (req, res) => {
  fs.writeFileSync('./build/src/apps/' + req.body.path, req.body.content, { encoding: 'utf8' })
  res.json({ res: 'success' })
})

//delete a file
app.get('/deleteFile', cors(corsOptions), (req, res) => {
  fs.rmSync('./build/src/apps/' + req.query?.path)
  res.json({ res: 'success' })
})

//get file list inside a directory
app.get('/readFolder', cors(corsOptions), (req, res) => {
  const content = fs.readdirSync('./build/src/apps/' + req.query.path, { withFileTypes: true })
...
app.get('/newFile', cors(corsOptions), (req, res) => {
  const exists = fs.existsSync('./build/src/apps/'  + req.query.path)
  if(exists) {
    res.json({res: 'File Already Exists'})
  } else {
    fs.writeFileSync('./build/src/apps/' + req.query.path, '', 'utf8')
    res.json({res: 'success'})
  }
})
...
app.get('/newProject', cors(corsOptions), (req, res) => {
  try {
    if(req.query?.id) {
      ...
      fs.mkdirSync('./build/src/apps/' + id)
      fs.mkdirSync('./build/src/apps/' + id + '/src/components', { recursive: true })
      fs.mkdirSync('./build/src/apps/' + id + '/src/utils')
      fs.writeFileSync('./build/src/apps/' + id + '/src/index.js', indexTemplate)
      fs.writeFileSync('./build/src/apps/' + id + '/package.json', packageTemplate(req.query?.name))
      fs.writeFileSync('./build/src/apps/' + id + '/git.json', gitTemplate(req.query?.name))
      fs.writeFileSync('./build/src/apps/' + id + '/appearance.json', appearanceTemplate(req.query?.name, req.query?.id))
      res.json({res: 'success'})
    }
  } catch (err) {
    res.json({err})
...
app.get('/addFileForGithub', cors(corsOptions), async(req, res) => {
  try {
    let content = JSON.parse(fs.readFileSync('./build/src/apps/'+req.query.app+'/git.json', 'utf8'))
    ...
    if(!files.includes(globbedPath[0])) {
      ...
      fs.writeFileSync('./build/src/apps/'+req.query.app+'/git.json', JSON.stringify(content))
    }
    res.json({res: 'success'})
  } catch (e) {
    res.json({err: e})
...
app.get('/addDirForGithub', cors(corsOptions), async(req, res) => {
  try {
    let content = JSON.parse(fs.readFileSync('./build/src/apps/'+req.query.app+'/git.json', 'utf8'))
    const globbedPath = globbySync('./build/src/apps/' + req.query.app)
...
app.get('/createPublicFolder', cors(corsOptions), async(req, res) => {
  try {
    let content = JSON.parse(fs.readFileSync('./build/src/apps/'+req.query.app+'/appearance.json', 'utf8'))
    if(!fs.existsSync('./build/src/apps/'+req.query.app+'/public')) {
      fs.mkdirSync('./build/src/apps/'+req.query.app+'/public')
    }

    fs.writeFileSync('./build/src/apps/'+req.query.app+'/public/manifest.json', 
        ManifestJSON(content?.shortName, content?.name, content?.themeColor, content?.backgroundColor), 'utf-8')

    content?.robotsTxt && fs.writeFileSync('./build/src/apps/'+req.query.app+'/public/robots.txt', content?.robotsTxt, 'utf-8')
    ...
    if(content?.icon) {
      ...
      fs.writeFileSync('./build/src/apps/'+req.query.app+'/public/icon.' + type, base64Data, 'base64')

      delete content.icon
      fs.writeFileSync('./build/src/apps/'+req.query.app+'/appearance.json', JSON.stringify(content, null, '\t'), 'utf8')
    }

    fs.writeFileSync('./build/src/apps/'+req.query.app+'/public/index.html', 
...
app.get('/cleanPublicFolder', cors(corsOptions), async(req, res) => {
  try {
    fs.rmdirSync('./build/src/apps/'+req.query.app+'/public', {
      force: true,
      recursive: true
    })
...
io.on('connection', (socket) => {
  ...
  fs.watch('./build/src/apps/').on('change', (event, file) => {
    if(fs.existsSync('./build/src/apps/' + file)) {
